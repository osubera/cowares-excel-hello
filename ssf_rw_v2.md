...
## 概要 ##
  * SSF テキストとエクセルブックの間を変換するツール、２号機

# Details #

![http://4.bp.blogspot.com/-EDfMXcoX1f8/Tb4pscXoCJI/AAAAAAAAAFU/hU2ETtUzks4/s1600/shot1.png](http://4.bp.blogspot.com/-EDfMXcoX1f8/Tb4pscXoCJI/AAAAAAAAAFU/hU2ETtUzks4/s1600/shot1.png)

  * next version of [ssf\_reader\_primitive](ssf_reader_primitive.md) , [ssf\_writer\_primitive](ssf_writer_primitive.md) and [ssf\_rw\_primitive](ssf_rw_primitive.md) .
  * new features.
...
## 説明 ##
  * [ssf\_reader\_primitive](ssf_reader_primitive.md) と [ssf\_writer\_primitive](ssf_writer_primitive.md) と [ssf\_rw\_primitive](ssf_rw_primitive.md) の次バージョン。
  * 主な追加機能。
    1. ダイアログ付きのファイル入出力。
...
# Downloads #
  ...
  * [downloads / ダウンロード](http://code.google.com/p/cowares-excel-hello/downloads/list?can=2&q=ssf)

# How to use #

  1. download binary excel book, ssf\_rw\_v2.xls (Japanese Excel 2000) or ssf\_rw\_v2.xlsm (Japanese Excel 2007) , from above url.
    * you must enable your excel to run a macro, when you can't see a toolbar.
    * you must do [allow access to a VBA Project Object Model](vba#how_to_allow_access_to_a_Visual_Basic_project.md)  to handle VBA code.
  1. you may want to start with ssf\_rw\_v2\_en.xls (English Excel 2000) , if your PC doesn't handle Japanese language.
  1. build your book from text source, if you can't download binary books, or you are prohibited to download any macro book from the internet.
    1. First of all, copy and generate manually the [ssf\_reader\_primitive](ssf_reader_primitive.md) to a module of a new excel book.
    1. download a english source text file, or copy the code at below. then convert it to an excel book with the [ssf\_reader\_primitive](ssf_reader_primitive.md).
    1. while [ssf\_reader\_primitive](ssf_reader_primitive.md) doesn't change the project name, you have to do it manually. change `VBAProject` to `F3SsfRwV2` and save.

## 使い方 ##
  1. 上記、ダウンロード先から、 ssf\_rw\_v2.xls (Excel 2000) または ssf\_rw\_v2.xlsm (Excel 2007) をダウンロードして使う。
    * ブックを開いてもツールバーが現われないときは、エクセルでマクロが実行できるような設定を行う。
    * VBA コードを入出力するには、 [VBA プロジェクト オブジェクト モデルへのアクセスを信頼する](vba#Visual_Basic_%E3%83%97%E3%83%AD%E3%82%B8%E3%82%A7%E3%82%AF%E3%83%88%E3%81%B8%E3%81%AE%E3%82%A2%E3%82%AF%E3%82%BB%E3%82%B9%E3%82%92%E4%BF%A1%E9%A0%BC%E3%81%95%E3%81%9B.md) が必要。
  1. 日本語以外の環境で使うなら、 ssf\_rw\_v2\_en.xls (英語版 Excel 2000) をダウンロードして使う。
  1. バイナリのダウンロードや、マクロ付きブックの持込みができない場合、次の手順でソースから構築する。
    1. 最初に [ssf\_reader\_primitive](ssf_reader_primitive.md) を標準モジュールへの手動コピーで作成する。
    1. 次に SJIS 版または英語版のソーステキストをダウンロードするか、下のコードをコピーし、 [ssf\_reader\_primitive](ssf_reader_primitive.md) でエクセルブックに変換する。
    1. [ssf\_reader\_primitive](ssf_reader_primitive.md) では、 VBA プロジェクト名を自動設定しないため、変換後に手動で `VBAProject` を `F3SsfRwV2` に変更してから保存する。


# Interface / 操作 #

![http://2.bp.blogspot.com/-VFlDVjhi6aM/Tb4ps6zYJ5I/AAAAAAAAAFY/kpJ43ab8G34/s1600/shot2.png](http://2.bp.blogspot.com/-VFlDVjhi6aM/Tb4ps6zYJ5I/AAAAAAAAAFY/kpJ43ab8G34/s1600/shot2.png)

  1. **Copy as SSF** | Write SSF into Clipboard
  1. **Paste from SSF** | Read SSF from Clipboard
  1. **to Notepad** | Write SSF into a Notepad
  1. **from Notepad** | Read SSF from a Notepad
  1. **0** | connection to Notepad
  1. **to File** | Write SSF into a File
  1. **from File** | Read SSF from a File
  1. **Encoding** | Character Encoding of File
  1. **Object** | What is converted when writing to SSF
  1. **Module** | How VBA codes are converted
  1. **Mixed** | use SSF declares to remove non SSF sections
  1. **Log** | Show verbose logs on Notepad
  1. **?** | Show an information

  1. **コピーSSF** | クリップボードにSSFを書き出す。
  1. **ペーストSSF** | クリップボードのSSFを取り込む。
  1. **メモ帳へ** | メモ帳にSSFを書き出す。
  1. **メモ帳から** | メモ帳のSSFを取り込む。
  1. **0** | 連携中のメモ帳。
  1. **ファイルへ** | ファイルにSSFを書き出す。
  1. **ファイルから** | ファイルのSSFを取り込む。
  1. **文字コード** | ファイルの文字コードを指定する。
  1. **対象** | SSF書き出しの対象を指定する。
  1. **モジュール** | VBAコードの扱いを指定する。
  1. **混在** | SSF宣言行より前の情報を取り込まない。
  1. **ログ** | メモ帳にログを出力する。
  1. **?** | 情報シートを表示する。

# Snapshots #

![http://1.bp.blogspot.com/-wrRgIjFfAH0/Tb4ptoXW7tI/AAAAAAAAAFc/B4hCdrbdXSQ/s1600/shot3.png](http://1.bp.blogspot.com/-wrRgIjFfAH0/Tb4ptoXW7tI/AAAAAAAAAFc/B4hCdrbdXSQ/s1600/shot3.png)
