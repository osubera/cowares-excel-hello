...
## 説明 ##
  ...
  * 入力フォームの概要
    * 左右に２つのテキスト枠が並ぶ。
      * 左側が編集用。右側が参照用。
    * 上部に、操作ボタンが並ぶ。
      * 比較(&C)、保存(&W)、次(&A)、↓(&X)、↑(&E)、←(&S)、→(&D)、閉じる(&Q)、隠す(&Z)、切替(&R)
  * 入力フォームの機能
    * 比較機能
...
# Downloads #
  ...
  * [downloads / ダウンロード](http://code.google.com/p/cowares-excel-hello/downloads/list?can=2&q=compare_sheet_form)
...
## 使い方 ##
  ...
  * 用途
    1. フォームでセルの長文を編集する。
    1. ２つのワークシートで、内容が一致しないセルを探す。
    1. 参照用シートと編集用シートの、対応するセルをフォームで比べながら、長文を編集する。

  1. フォームでの編集
    1. ツールバーの `入力` ボタンを押すと、現在のセルをフォームで編集する。
    1. `F2` キーを、 `入力` ボタンのショートカットとして使用できる。
  1. 不一致セルの検索
    1. ツールバーの `参照シート` ボタンを押すと現在のシートを参照用（１次）シートとして記憶する。
    1. ツールバーの `編集シート` ボタンを押すと現在のシートを編集用（２次）シートとして記憶する。
    1. ツールバーの `比較` ボタンを押すと、２つのシートを比較し、ボタンの右側の結果表示枠に結果を表示する。
      * すべて一致している場合、 `○` が表示される。
      * 一致しないセルがあれば、その数が表示される。
      * 一致しないセルのリストが、数の下に並び、選んだセルにジャンプし、フォームで編集できる。
  1. 比較しながら編集
    1. 参照シートと編集シートが設定された状態でフォームを開くと、対応するセルの内容を並べて表示し、比較編集ができる。
    1. 比較した結果に応じて、参照側の色が変化する。
      * 参照セルと編集セルの内容が一致すれば、白地に黒文字。
      * 一致しないとき、背景が黄色になる。
      * 一致しない最初の文字以降が赤文字になる。
  1. フォームの操作
    1. `比較` ボタンは、編集中のテキスト（保存前）と参照セルを比較し、その結果に応じて色を変更する。
    1. `保存` ボタンは、編集中のテキストを編集セルに書き込む。
    1. `次` ボタンは、不一致セルの検索結果リストを使って、不一致セルを順に編集する。
    1. `↓` `↑` `←` `→` ボタンは、隣接するセルを編集する。
    1. `閉じる` ボタンは、フォームを閉じる。
    1. `隠す` ボタンは、フォームを非表示にする。（編集を継続したまま、一時的にエクセル本体を表示する。）
    1. `切替` ボタンは、タスクバーが固まってブック切り替えができなくなる現象を解除する。
    1. すべてのボタンで、 Alt キーによるショートカットが利用できる。
  1. フォームのサイズと移動
    1. フォームはエクセル本体と同じサイズで開く。
    1. マウスおよび、最小化、最大化ボタンで大きさを変更できる。
      * 最小化ボタンでは、タスクバーに格納されるのではなく、画面左下に小さくなる。これがタスクバー等に隠れてクリックできなくなった場合、ツールバーの `入力` ボタンを押すと、元にもどる。
    1. ツールバーの `解除` ボタンを押すまで、直前のサイズを覚えている。
    1. フォームは自由に移動できる。最初はエクセル本体にかぶせて開くが、移動すれば、その場所を記憶する。
    1. フォームは常に前面表示されるが、移動させれば開いたままでもエクセルの操作ができる。
  1. その他の機能
    1. 隠したフォームを再表示するには、ツールバーの `入力` ボタンを押す。
    1. ツールバーの `解除` ボタンを押すと、すべての設定を消去しフォームを閉じる。
    1. `切替` ボタンの機能は、 `閉じる` `隠す` `解除` にも割り当てられている。
      * これは、エクセル2007のウィンドウをタスクバーに表示する機能とユーザーフォームの相性でブックが切り替わらなくなる問題を回避する機能。
      * [issue 15](https://code.google.com/p/cowares-excel-hello/issues/detail?id=15)

### 2011/4/6 版での修正 ###
  1. ショートカットを F3 から F2 へ。
  1. フォームリサイズを可能に。
  1. 切替ボタンを追加。
  * ![http://3.bp.blogspot.com/-AkAqjhoeTBI/TZwERxNH0xI/AAAAAAAAAEk/t23Wulw5VUQ/s1600/shot8.png](http://3.bp.blogspot.com/-AkAqjhoeTBI/TZwERxNH0xI/AAAAAAAAAEk/t23Wulw5VUQ/s1600/shot8.png)
...
> > ![http://1.bp.blogspot.com/-_m2YlWm2PMs/TZwERSpgECI/AAAAAAAAAEg/soCwZvB8bHk/s1600/shot7.png](http://1.bp.blogspot.com/-_m2YlWm2PMs/TZwERSpgECI/AAAAAAAAAEg/soCwZvB8bHk/s1600/shot7.png)
> > ![http://4.bp.blogspot.com/-qWcfzFYPLQ8/TZfrm4n318I/AAAAAAAAAEc/Lzc-ZnyFet0/s1600/shot6.png](http://4.bp.blogspot.com/-qWcfzFYPLQ8/TZfrm4n318I/AAAAAAAAAEc/Lzc-ZnyFet0/s1600/shot6.png)

**エクセル2000 ではツールバーを表示する。

> ![http://4.bp.blogspot.com/-ZWRa3yS9hkE/TZfrksvJYoI/AAAAAAAAAEI/28brFTHPglU/s1600/shot1.png](http://4.bp.blogspot.com/-ZWRa3yS9hkE/TZfrksvJYoI/AAAAAAAAAEI/28brFTHPglU/s1600/shot1.png)**

**ツールバーの比較結果から、不一致セルにジャンプする。
> ![http://3.bp.blogspot.com/-ciZj2nVkjrI/TZfrlYZJniI/AAAAAAAAAEQ/VrwSWRDbi-k/s1600/shot3.png](http://3.bp.blogspot.com/-ciZj2nVkjrI/TZfrlYZJniI/AAAAAAAAAEQ/VrwSWRDbi-k/s1600/shot3.png)**

**フォームで比較しながら編集する。
> ![http://3.bp.blogspot.com/-AkAqjhoeTBI/TZwERxNH0xI/AAAAAAAAAEk/t23Wulw5VUQ/s1600/shot8.png](http://3.bp.blogspot.com/-AkAqjhoeTBI/TZwERxNH0xI/AAAAAAAAAEk/t23Wulw5VUQ/s1600/shot8.png)
> ![http://3.bp.blogspot.com/-YlJiwRP5b0c/TZfrkxoq4mI/AAAAAAAAAEM/P1aabjFuJy4/s1600/shot2.png](http://3.bp.blogspot.com/-YlJiwRP5b0c/TZfrkxoq4mI/AAAAAAAAAEM/P1aabjFuJy4/s1600/shot2.png)**

**エクセル本体と並べると、このようにシートと連携している。
> ![http://2.bp.blogspot.com/-_Fh5CI9X3FA/TZfrlz0hNvI/AAAAAAAAAEU/nvWM3q3V4mU/s800/shot4.png](http://2.bp.blogspot.com/-_Fh5CI9X3FA/TZfrlz0hNvI/AAAAAAAAAEU/nvWM3q3V4mU/s800/shot4.png)**

**編集だけのときは、フォーム全体を広く使う。
> ![http://3.bp.blogspot.com/-woINKXEhoUc/TZfrmXU4tsI/AAAAAAAAAEY/JjT3_RlIdhU/s800/shot5.png](http://3.bp.blogspot.com/-woINKXEhoUc/TZfrmXU4tsI/AAAAAAAAAEY/JjT3_RlIdhU/s800/shot5.png)**
...
'workbook
'   name;compare_sheet_form.xls/F3CompareSheetForm
...
'worksheet
'   name;compare_sheet_form/BaumMain
...
'cells-name
'       ;=compare_sheet_form!R15C1
'       ;compare_sheet_form!_ButtonCaption
'       ;=compare_sheet_form!R3C2
'       ;compare_sheet_form!_Comment
'       ;=compare_sheet_form!R6C2
'       ;compare_sheet_form!_Contributor
'       ;=compare_sheet_form!R4C2
'       ;compare_sheet_form!_Copyright
'       ;=compare_sheet_form!R5C2
'       ;compare_sheet_form!_License
'       ;=compare_sheet_form!R2C2
'       ;compare_sheet_form!_LocalComment
'       ;=compare_sheet_form!R1C2
'       ;compare_sheet_form!_PublicName
'       ;=compare_sheet_form!R7C2
'       ;compare_sheet_form!_Since
'       ;=compare_sheet_form!R10C2
'       ;compare_sheet_form!_Tag
'       ;=compare_sheet_form!R9C2
'       ;compare_sheet_form!_Timestamp
'       ;=compare_sheet_form!R8C2
'       ;compare_sheet_form!_Url
...
Friend Function VBProjectName() As String
    ...
    VBProjectName = "F3CompareSheetForm"
End Function
...
'module
'   name;CompareSheetForm
'{{{
Option Explicit

Const ShowFormKey = "{F2}"
...
Public Sub ShowVerifyCharForm()
    On Error Resume Next
    If VerifyChar.PseudoOpen Then VerifyChar.Show vbModeless
End Sub

Public Sub CloseVerifyCharForm()
    On Error Resume Next
    Unload VerifyChar
End Sub

Public Sub EnableShortCutKey()
    Application.OnKey ShowFormKey, BaumMain.VBProjectName & ".CompareSheetForm.ShowVerifyCharForm"
End Sub
...
Private Sub UserForm_Initialize()
    ResizeEnabled = False
    Set CompareHelper = BaumMain.GetCompareHelper
    OnFormLoad
    PseudoOpen
End Sub
...
Private Sub UserForm_Layout()
    If Not ResizeEnabled Then
        ...
        CompareSheetForm.MakeFormResizable Me
    End If
    With Me
...
Private Sub cEdit_Change()
    If CompareHelper.Saved Then CompareHelper.Saved = False
End Sub
...
Private Sub cRefer_DocumentComplete(ByVal pDisp As Object, URL As Variant)
    'Debug.Print Timer(), "DocumentComplete", URL
    ' signal to update
    cRefer.StatusBar = False
End Sub

Private Sub cRefer_OnStatusBar(ByVal StatusBar As Boolean)
    'Debug.Print Timer(), "OnStatusBar", StatusBar
    If StatusBar Then Exit Sub
...
Private Sub cCompare_Click()
    CompareHelper.OnCompareText
End Sub

Private Sub cWrite_Click()
    If CompareHelper.OnWriteText Then
        CompareHelper.OnCompareText
    End If
End Sub

Private Sub cClose_Click()
    If CompareHelper.OnBeforeCellMoveOrClose() Then
        FocusOnEdit
    Else
        PseudoClose
    End If
...
Private Sub cNext_Click()
    ...
    CompareHelper.JumpNext
End Sub

Private Sub cDown_Click()
    CompareHelper.OnCellMoveTo 1, 0
End Sub
...
Private Sub cLeft_Click()
    CompareHelper.OnCellMoveTo 0, -1
End Sub
...
Private Sub cCompare_Enter()
    OnButtonFocus cCompare
End Sub

Private Sub cCompare_Exit(ByVal Cancel As MSForms.ReturnBoolean)
    OnButtonUnFocus cCompare
End Sub

Private Sub cNext_Enter()
    OnButtonFocus cNext
End Sub

Private Sub cNext_Exit(ByVal Cancel As MSForms.ReturnBoolean)
    OnButtonUnFocus cNext
End Sub
...
Private Sub cCompare_KeyDown(ByVal KeyCode As MSForms.ReturnInteger, ByVal Shift As Integer)
    OnKeyDown KeyCode, Shift
End Sub
...
Private Sub cNext_KeyDown(ByVal KeyCode As MSForms.ReturnInteger, ByVal Shift As Integer)
    OnKeyDown KeyCode, Shift
End Sub
...
Public Function PseudoOpen() As Boolean
    If CompareHelper.Editing Then
        PseudoOpen = True
        Exit Function
    End If
    
    If Not CompareHelper.BeginEdit Then
        PseudoOpen = False
        Exit Function
    End If
    
    OnFormResize GetPreferredWidth, GetPreferredHeight
    CompareHelper.SetStatusOn
    PseudoOpen = True
End Function

Public Sub PseudoClose()
    CompareHelper.SetStatusOff
    CompareHelper.EndEdit
    PseudoHide
End Sub
...
Public Sub UpdateReferBox(Address As String)
    cReferAddress.Caption = Address
    With cRefer
        ' ie is busy when the StatusBar is set true
        If .StatusBar Then Exit Sub
        
        If .Document Is Nothing Then
            InitializeReferBox
        Else
            ' signal to update. setting false over false is enough to fire an event
            .StatusBar = False
        End If
    End With
End Sub

Public Sub UpdateEditBox(Address As String, Text As String, PreferredFont As Font)
...
Private Sub InitializeReferBox()
    With cRefer
        ' use the statusbar visible as a signal to initialize
        ' true = initializing
        ' false = ready to use
        .StatusBar = True
...
Private Sub UpdateReferBoxText()
    CompareHelper.UpdateReferBoxText cRefer.Document
End Sub

Private Sub SetPreferredFont(PreferredFont As Font)
    On Error Resume Next
    
    With cEdit.Font
        .Name = PreferredFont.Name
        .Size = PreferredFont.Size
    End With
End Sub
...
Private Sub OnFormResize(ByVal Width As Single, ByVal Height As Single)
    ...
    cNext.Enabled = CompareHelper.HasSummaryList
    cCompare.Enabled = CompareHelper.HasReferSheet
    If cCompare.Enabled Then BoxWidth = Int(BoxWidth / 2)
    Left2 = Me.InsideWidth - BoxWidth
    
    With cEdit
        BoxHeight = BoxHeight - .Top
    ...
    If CompareHelper.HasReferSheet Then
        With cRefer
            .Height = BoxHeight
            .Width = BoxWidth
            .Left = Left2
        ...
        End With
    Else
        cRefer.Visible = False
        cReferAddress.Visible = False
    End If
End Sub
...
Public Sub SetReferenceSheet(Ws As Worksheet, oAC As Office.CommandBarButton)
    If Ws Is EditSheet Then ClearEditSheet
...
Public Sub SetEditSheet(Ws As Worksheet, oAC As Office.CommandBarButton)
    If Ws Is ReferSheet Then ClearReferSheet
    If Not EditButton Is oAC Then SetHelperSettings
    ClearSummaries
    Set EditSheet = Ws
    SetupButton EditButton, EditSheet
End Sub
...
Public Sub JumpTo()
    ...
    With SummaryButton
        ...
        If UserForms.Count > 0 Then
            If Not VerifyChar.cNext.Enabled Then VerifyChar.cNext.Enabled = True
        End If
