...
# Details #
  ...
  * let the quick sort function in [hello\_sort](hello_sort.md) use a Collection Object instead of an array.
  * it works, but the performance is worse than expected.
  * see [bad\_collection](bad_collection.md) for the reason.

## 説明 ##
  * [hello\_sort](hello_sort.md) のクイックソート関数で、配列の代わりに Collection オブジェクト上でのソートをやってみる。
  * うまく動作するが、速度は思ったより遅い。
  * [bad\_collection](bad_collection.md) に理由がある。
