...
# Details #
  ...
  * a toolbar appears when you open the book. (buttons in the Addin Ribbon for Excel 2007 or later)
    * buttons appears on starting excel, if activated as an addin.
  * open a book to be validated, select cells, rows, or columns to be validated, then press the `検証` button.
  * validates,
    * cell does not have any hankaku characters.
    * not for validation but also check zenkaku alphabet and number characters to be informed.
  * cells invalidated are colored, also characters are colored.
  * summary is available at toolbar.

## 説明 ##
  * ブックを開くと、操作用のボタンがツールバー（リボン）に追加される。
    * アドインとして有効にすると、エクセル起動時にボタンが追加される。
...
# Downloads #
  ...
  * [downloads / ダウンロード](http://code.google.com/p/cowares-excel-hello/downloads/list?can=2&q=validate_char_zenkaku)

# How to use #

  1. select columns or so to be validated.
  1. push the validate button.
  1. a number of cells invalidated are displayed at a toolbar list.
  1. the list also contains addresses of cells invalidated, to jump quickly.
  1. character markers
    1. hankaku : red
    1. zenkaku alphabet number : green
    1. others : black
    1. a continuous hankaku characters with spaces is underscored, that makes the space visible.

## 使い方 ##
  1. 検証対象の列などを選択する。
  1. 検証ボタンを押す。
  1. 検証に合格しないセルがあると、ツールバーのリストに不合格セルの件数が表示される。
  1. リストに不合格セルの位置が入っており、選択により該当セルに移動できる。
  1. 文字マークの種類
    1. 半角文字　　：　赤色
    1. 全角英数字　：　赤色
    1. その他　　　：　黒色
    1. 半角文字が半角スペースを含む場合、スペースが見えるよう、下線をつける。

| **文字の種類** | **文字の色** | **セルの色** | **エラーとしてカウント** | **アンダーライン** |
|:----------|:---------|:---------|:---------------|:------------|
| 半角スペース    | 赤        | 黄        | する             | あり          |
| 半角文字      | 赤        | 黄        | する             | なし          |
| 全角英数      | 赤        | ペールブルー(93cddd) ColorIndex 37 | しない            | なし          |
| その他       | 黒        | 白        | しない            | なし          |

  * 文字種類が混在するときの動作は、表の上位が優先

### 2011/4/13 の修正 ###

  1. アドインとして動くようにする。
    * 機能は今のブックと同じで、アドインとして保存できるよう、内部の調整をする。
    * ブックとアドインの両方を公開する。
    * 開発者用メモ：　プロジェクト名、ツールバーParent、ブックプロパティ
  1. 全角`￥`を、全角英数として扱う。
    * 今のブックでは、半角`\`は半角文字として検出する一方で、全角`￥`は漢字として検出するため、全角英数チェックにかからない。
    * これは不具合として修正する。

# Snapshots #

  * validated as good / 検証により問題が無いことがわかった。
> > ![https://lh6.googleusercontent.com/-HrCO_FR32ks/TWuNGpP0RAI/AAAAAAAAADk/hmJXymMog2A/s1600/shot2.png](https://lh6.googleusercontent.com/-HrCO_FR32ks/TWuNGpP0RAI/AAAAAAAAADk/hmJXymMog2A/s1600/shot2.png)
  * 8 cells are invalid / 検証により８つのセルに問題があった。
> > ![https://lh5.googleusercontent.com/-oZA0eTj2i6Y/TWuNGKKRNDI/AAAAAAAAADg/RRgZXORAGOE/s1600/shot1.png](https://lh5.googleusercontent.com/-oZA0eTj2i6Y/TWuNGKKRNDI/AAAAAAAAADg/RRgZXORAGOE/s1600/shot1.png)
...
'workbook
'   name;validate_char_zenkaku.xls/F3ValidateCharZenkaku
...
Friend Function VBProjectName() As String
    ...
    VBProjectName = "F3ValidateCharZenkaku"
End Function
...
Public Sub DoValidate(Target As Object, Summary As Object, Helper As ToolBarV2)
    ...
    Const ZenkakuAlphaColor = 3
    Const AlarmCellColor = 6
    Const ZenkakuCellColor = 37
    ...
    For Each ACe In TaRa.Cells
        ...
        If Rzenan.Test(AceText) Then
            ACe.Interior.ColorIndex = ZenkakuCellColor
