...
![http://4.bp.blogspot.com/_EUW0nrj9XlM/TRXbPYr-cfI/AAAAAAAAAAg/udfOZmYLlMI/s1600/shot1.png](http://4.bp.blogspot.com/_EUW0nrj9XlM/TRXbPYr-cfI/AAAAAAAAAAg/udfOZmYLlMI/s1600/shot1.png)

  1. **copy ssf** | copy activebook as ssf to clipboard
  1. **paste ssf** | paste to excel from clipboard ssf text
  1. **to notepad** | open a windows notepad contains an ssf converted from activebook
  1. **from notepad** | convert to excel from current notepad ssf text
  1. **0** | show the current notepad
  1. **about ssf\_rw\_primitive** | show an information

  1. **コピーSSF** | アクティブブックを変換し、クリップボードにSSFを書き出す。
  1. **ペーストSSF** | クリップボードのSSFを取り込み、エクセルブックにする。
  1. **メモ帳へ** | アクティブブックを変換し、メモ帳にSSFを書き出す。
  1. **メモ帳から** | メモ帳のSSFを取り込み、エクセルブックにする。
  1. **0** | 連携中のメモ帳を表示する。
  1. **ssf\_rw\_primitive について** | 情報シートを表示する。

# Snapshots #

![http://4.bp.blogspot.com/_EUW0nrj9XlM/TRXdqd49edI/AAAAAAAAAAk/xZ4vtHtysJw/s1600/shot1.png](http://4.bp.blogspot.com/_EUW0nrj9XlM/TRXdqd49edI/AAAAAAAAAAk/xZ4vtHtysJw/s1600/shot1.png)
