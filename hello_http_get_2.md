...
# Details #
  ...
  * perform web searches on Google, Bing and Yahoo on excel.
  * [hello\_http\_get](hello_http_get.md) , [hello\_urlencode](hello_urlencode.md) and [hello\_charset\_adodb\_stream](hello_charset_adodb_stream.md) give us everything required.
  * we use rough approaches to cut texts here, to avoid precise analysis of html source.
  * finally, we have some results of url and summary from 3 search engines.

## 説明 ##
  * グーグル、ビング、ヤフーのウェブ検索をエクセルから実行する。
  * [hello\_http\_get](hello_http_get.md) と [hello\_urlencode](hello_urlencode.md) と [hello\_charset\_adodb\_stream](hello_charset_adodb_stream.md) とに必要なものは全部ある。
  * ここでは、 HTML ソースの細かい解析をしないで、大雑把にテキストを切る手法を使った。
  * ３つの検索エンジンから、 URL と説明文を取り出すところまでできた。
...
# How to use #
  ...
  1. from test1() to test5() are executable examples.
  1. Worksheet `Search Keywords` contains words to search, try another word.

## 使い方 ##
  1. [ssf\_reader\_primitive](ssf_reader_primitive.md) のような ssf 読み込みツールを使って、下のコードをエクセルブックに変換する。
  1. test1() から test5() までが実行可能な見本。
  1. `Search Keywords` ワークシートに検索用の言葉がある。他の言葉に変えて試してみよう。

![http://2.bp.blogspot.com/_EUW0nrj9XlM/TSGeqSWcJDI/AAAAAAAAABA/UQ-IIU3qj9o/s1600/shot2.png](http://2.bp.blogspot.com/_EUW0nrj9XlM/TSGeqSWcJDI/AAAAAAAAABA/UQ-IIU3qj9o/s1600/shot2.png)
...
'worksheet
'  name;Search Keywords/SearchKeywords
...
Sub test1()
    ...
    'ActiveCell.Value = DecUrlParametersArray(s)
    'Selection.Value = DecUrlParametersArray(s)
    'ActiveCell.Value = p
    'Selection.Value = p
    Debug.Print EncVariantArray(Selection.Value)
End Sub
...
Sub test1()
    ...
    Url = GoogleSearchUrl("utf8 bom 消", 10)
    Debug.Print Url
    
    HtmlSource = GetWebText(Url)
    'Debug.Print HtmlSource
    Debug.Print LaserKnife(HtmlSource, "<b>utf8 bom 消</b>", 70)
End Sub

' Google 検索実行
Sub test2()
    Const engine = "Google"
    Const cut1 = "<td class=""j""><font size=""-1"">"
    Const cut2 = "/url?q="
    Const cut3 = "&amp;"
    ...
    Keywords = GetKeywordsFromWorksheet
    Keyword = CStr(Keywords(0))
    HtmlSource = GetWebText(GoogleSearchUrl(Keyword))
    
    Summary = LaserKnives(HtmlSource, cut1, 400)
    Url = LaserKnife(LaserKnives(HtmlSource, cut2, 1000), cut3, -1, True)
    
    Set Ws = AddNewWorksheet
    Ws.Columns("A:C").ColumnWidth = 40
    
    PublishToWorksheet Array(Keyword), Ws, 1, 1
    PublishToWorksheet Array(engine), Ws, 2, 1
    PublishToWorksheet Url, Ws, 3, 2
    PublishToWorksheet Summary, Ws, 3, 3
End Sub

' Bing 検索実行
Sub test3()
    Const engine = "Bing"
    Const cut1 = "<div id=""results"">"
    Const cut2 = "<li class=""sa_wr"">"
    Const cut3 = "</h3></div>"
    Const cut4 = "href="""
    Const cut5 = """"
    Const cut6 = "<div id=""sa_bop"">"
    ...
    Keywords = GetKeywordsFromWorksheet
    Keyword = CStr(Keywords(0))
    HtmlSource = GetWebText(BingSearchUrl(Keyword))
    
    v = LaserKnives(LaserKnife(LaserKnife(HtmlSource, cut1), cut6, -1, True), cut2)
    Url = LaserKnife(LaserKnife(v, cut4, -1, False, True), cut5, -1, True)
    Summary = LaserKnife(v, cut3, 400, False, True)
...
Sub test4()
    ...
    Const cut1 = "<div id=""web"">"
    Const cut2 = "<div id=""posS"" class=""spns"">"
    Const cut3 = "<li>"
    Const cut4 = "**"
    Const cut5 = """>"
    ...
    Keywords = GetKeywordsFromWorksheet
    Keyword = CStr(Keywords(0))
    HtmlSource = GetWebText(YahooJpSearchUrl(Keyword))
    
    v = LaserKnives(LaserKnife(LaserKnife(HtmlSource, cut1), cut2, -1, True), cut3)
    Summary = LaserKnife(v, cut5, 400, False, True)
    UrlEnc = LaserKnife(LaserKnife(v, cut4, -1, False, True), cut5, -1, True)
    ReDim Url(LBound(UrlEnc) To UBound(UrlEnc))
    For i = LBound(UrlEnc) To UBound(UrlEnc)
        Url(i) = DecUrlString(CStr(UrlEnc(i)))
    Next
...
' Search Keywords シートの検索語を Google で検索する
Sub test5()
    Const engine = "Google"
    Const cut1 = "<td class=""j""><font size=""-1"">"
    Const cut2 = "/url?q="
    ...
    For Each Keyword In Keywords
        HtmlSource = GetWebText(GoogleSearchUrl(CStr(Keyword)))
        
        Summary = LaserKnives(HtmlSource, cut1, 400)
        Url = LaserKnife(LaserKnives(HtmlSource, cut2, 1000), cut3, -1, True)
        
        RowCounter = PublishToWorksheet(Array(Keyword), Ws, RowCounter, 1)
        PublishToWorksheet Url, Ws, RowCounter, 2
        RowCounter = PublishToWorksheet(Summary, Ws, RowCounter, 3)
    Next
End Sub
...
Function GoogleSearchUrl(SearchText As String, Optional Start As Long = 0, Optional Language As String = "ja") As String
    Const Url As String = "http://www.google.co.jp/search"
    Dim out As String
    Dim Parameters As Variant
    Dim StartPage As Variant
    
    Parameters = Array(Array("q", SearchText), Array("hl", Language))
    out = Url & "?" & EncUrlParameters(Parameters)
    
    If Start > 0 Then   ' 10 records begins at the next of "Start"
        StartPage = Array(Array("start", Start))
        out = out & "&" & EncUrlParameters(StartPage)
    End If
...
Function BingSearchUrl(SearchText As String, Optional Start As Long = 0, Optional Language As String = "ja") As String
    Const Url As String = "http://www.bing.com/search"
    Dim out As String
    Dim Parameters As Variant
    Dim StartPage As Variant
    Dim SearchWithLanguage As String
    
    If Language = "" Then
        SearchWithLanguage = SearchText
    Else
        SearchWithLanguage = SearchText & " language:" & Language
    End If
    
    Parameters = Array(Array("q", SearchWithLanguage), Array("FORM", "PORE"))
    out = Url & "?" & EncUrlParameters(Parameters)
    
    If Start > 0 Then   ' 5 or 10 records begins at the next of "Start"
        StartPage = Array(Array("first", Start + 1))
        out = out & "&" & EncUrlParameters(StartPage)
    End If
...
Function YahooJpSearchUrl(SearchText As String, Optional Start As Long = 0, Optional Language As String = "ja") As String
    Const Url As String = "http://search.yahoo.co.jp/search"
    Dim out As String
    Dim Parameters As Variant
    Dim StartPage As Variant
    Dim SetLanguages As Variant
    
    Parameters = Array(Array("p", SearchText), Array("ei", "UTF-8"))
    out = Url & "?" & EncUrlParameters(Parameters)
    
    If Start > 0 Then   ' 10 records begins at the next of "Start"
        StartPage = Array(Array("b", Start + 1))
        out = out & "&" & EncUrlParameters(StartPage)
    End If
    
    If Language <> "" Then
        SetLanguages = Array(Array("fl", "1"), Array("vl", "lang_" & Language))
        out = out & "&" & EncUrlParameters(SetLanguages)
    End If
    
    YahooJpSearchUrl = out
End Function
...
Function AddNewWorksheet() As Worksheet
    Dim Ws As Worksheet
    
    Set Ws = ThisWorkbook.Worksheets.Add(Before:=ThisWorkbook.Sheets(1))
    Ws.Name = Format(Now(), "y hnnss")
    
    Set AddNewWorksheet = Ws
End Function

Function PublishToWorksheet(Data As Variant, Ws As Worksheet, RowBeginsAt As Long, Optional ColBeginsAt As Long = 1) As Long
    Dim DataRows As Long
    Dim i As Long
    
    DataRows = UBound(Data) - LBound(Data) + 1
    For i = 0 To DataRows - 1
        Ws.Cells(RowBeginsAt + i, ColBeginsAt).Value = Data(i + LBound(Data))
    Next
    
    PublishToWorksheet = RowBeginsAt + DataRows     ' indicates next row
End Function

Function GetKeywordsFromWorksheet() As Variant
    Dim SearchTable As Range
    Dim out() As String
    Dim i As Long
    
    GetKeywordsFromWorksheet = Array()
    With SearchKeywords
        If .Range("A1").Value = "" Then Exit Function
        If .Range("A2").Value = "" Then
            GetKeywordsFromWorksheet = Array(.Range("A1").Text)
        Else
            Set SearchTable = .Range("A1", .Range("A1").End(xlDown))
            ReDim out(0 To SearchTable.Cells.Count - 1)
            For i = 0 To UBound(out)
                out(i) = SearchTable(i + 1).Text
            Next
            GetKeywordsFromWorksheet = out
        End If
    End With
End Function
'}}}
...
```
http://www.google.co.jp/search?q=utf8+bom+%E6%B6%88&hl=ja&start=10
 の検索結果 約 <b>2,400</b> 件中 <b>11</b> - <b>20</b> 件目 (<b>0.19</b> 秒)</fon
```

from test2() to test5() generates a new worksheet to store the results.
> test2() から test5() までは、新規ワークシートに結果を格納する。

![http://3.bp.blogspot.com/_EUW0nrj9XlM/TSGeptBFabI/AAAAAAAAAA8/gJMqFC3h-0g/s1600/shot1.png](http://3.bp.blogspot.com/_EUW0nrj9XlM/TSGeptBFabI/AAAAAAAAAA8/gJMqFC3h-0g/s1600/shot1.png)
