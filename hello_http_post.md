...
# Details #
  ...
  * see also [hello\_http\_get](hello_http_get.md) , [hello\_urlencode](hello_urlencode.md) and [hello\_charset\_adodb\_stream](hello_charset_adodb_stream.md) .
...
## 説明 ##
  ...
  * [hello\_http\_get](hello_http_get.md) と [hello\_urlencode](hello_urlencode.md) と [hello\_charset\_adodb\_stream](hello_charset_adodb_stream.md) も参考に。
...
# How to use #
  ...
  1. from test1() to test3() in Translations module are executable examples.
  1. Worksheet `Sheet1` contains some Japanese to be translated.
  1. you may want to run Sheet1.fix\_imported\_linefeed() before tests, because current [ssf\_reader\_primitive](ssf_reader_primitive.md) cannot import a multiline cell correctly.

## 使い方 ##
  1. [ssf\_reader\_primitive](ssf_reader_primitive.md) のような ssf 読み込みツールを使って、下のコードをエクセルブックに変換する。
  1. Translations 標準モジュールにある test1() から test2() までが実行可能な見本。
  1. `Sheet1` ワークシートに翻訳用の日本語がある。
  1. Sheet1.fix\_imported\_linefeed() をテスト前に実行しておくとよい。今の [ssf\_reader\_primitive](ssf_reader_primitive.md) は複数行のセルデータを正確に読み込めないから。
...
![http://2.bp.blogspot.com/_EUW0nrj9XlM/TSPt_rvL6DI/AAAAAAAAABE/uVmy_iVhif0/s1600/shot1.png](http://2.bp.blogspot.com/_EUW0nrj9XlM/TSPt_rvL6DI/AAAAAAAAABE/uVmy_iVhif0/s1600/shot1.png)