...
## 説明 ##
  ...
  * 入力フォームの機能
    ...
    * フォントサイズ変更。
      ...
      * 初期サイズは12pt。
      * サイズ変更を行う都度、2pt刻みでフォントサイズが大きく、または小さくなる。
      * フォントサイズは0以下にはならない。
    * フォント名
      * 編集枠で使用するフォントは、フォームを開いたときに自動設定され、個別セルに連動しての変化はしない。
      * 日本語のエクセルブックでは、ＭＳ Ｐゴシック が基本。
    * 表示情報
      * 編集中のセルアドレス
      * 変更ありを示すアスタリスク(`*`)マーク
      * フォーム上に入力されている文字数
        * 改行を２文字とカウントするので、ワークシート上でのLen()関数と厳密には一致しない。
    * 保存時のセル移動
...
**[downloads / ダウンロード](http://code.google.com/p/cowares-excel-hello/downloads/list?can=2&q=edit_cell_form)**
...
## 使い方 ##
  ...
  * 用途
    1. フォームでセルの長文を編集する。
    1. 数式があるセルは読み取り専用となるため、データだけを修正したい用途に最適。

  1. フォームでの編集
    1. ツールバーの `入力` ボタンを押すと、現在のセルをフォームで編集する。
    1. `F3` キーを、 `入力` ボタンのショートカットとして使用できる。
  1. フォームの操作
    1. `保存して閉じる` ボタンは、編集中のテキストを編集セルに書き込み、フォームを閉じる。
    1. `保存` ボタンは、編集中のテキストを編集セルに書き込む。
    1. `↓` `↑` `←` `→` ボタンは、隣接するセルを編集する。
    1. `閉じる` ボタンは、フォームを閉じる。
    1. `隠す` ボタンは、フォームを非表示にする。（編集を継続したまま、一時的にエクセル本体を表示する。）
    1. `切替` ボタンは、タスクバーが固まってブック切り替えができなくなる現象を解除する。
    1. `小` ボタンは、フォームのフォントを一回り小さくする。
    1. `大` ボタンは、フォームのフォントを一回り大きくする。
  ...
  1. フォームのサイズと移動
    ...
    1. ツールバーの `リセット` ボタンを押すまで、直前のサイズを覚えている。
  ...
  1. その他の機能
    ...
    1. ツールバーの `リセット` ボタンを押すと、すべての設定を消去しフォームを閉じる。
    1. `切替` ボタンの機能は、 `閉じる` `隠す` `リセット` にも割り当てられている。
...
> ![http://1.bp.blogspot.com/-nYqFE-hmQjM/T7eG3iVDgCI/AAAAAAAAAHQ/9CTAoT-jxU4/s1600/shot5.png](http://1.bp.blogspot.com/-nYqFE-hmQjM/T7eG3iVDgCI/AAAAAAAAAHQ/9CTAoT-jxU4/s1600/shot5.png)**

**エクセル2007 ではリボンのアドインタブにボタンを表示する。
> ![http://3.bp.blogspot.com/-X9gAq3UTYVI/T7eHDEo0Q1I/AAAAAAAAAHY/uICvDp31fxA/s1600/shot4.png](http://3.bp.blogspot.com/-X9gAq3UTYVI/T7eHDEo0Q1I/AAAAAAAAAHY/uICvDp31fxA/s1600/shot4.png)**

**セルの長文をフォームで編集する。
> ![http://4.bp.blogspot.com/-2Sgexn2WZXQ/T7eHIRBXrxI/AAAAAAAAAHg/CCkmo8ziXD0/s1600/shot1.png](http://4.bp.blogspot.com/-2Sgexn2WZXQ/T7eHIRBXrxI/AAAAAAAAAHg/CCkmo8ziXD0/s1600/shot1.png)**

**数式の計算結果が日付の場合。
> ![http://2.bp.blogspot.com/-sSdoY01SaIM/T7eHK1RsYSI/AAAAAAAAAHo/3AHt116iDU0/s1600/shot2.png](http://2.bp.blogspot.com/-sSdoY01SaIM/T7eHK1RsYSI/AAAAAAAAAHo/3AHt116iDU0/s1600/shot2.png)**

**数式の計算結果が数値の場合。
> ![http://3.bp.blogspot.com/-sJEBBKfpRhA/T7eHMzNUbtI/AAAAAAAAAHw/b4uTvkoy9E0/s1600/shot3.png](http://3.bp.blogspot.com/-sJEBBKfpRhA/T7eHMzNUbtI/AAAAAAAAAHw/b4uTvkoy9E0/s1600/shot3.png)**
...
'workbook
'   name;edit_cell_form.xlsm/F3EditCellForm
...
Private Sub OnBeginEditCell()
    ...
    CellEditor.UpdateValueBox CurrentCell.Value, ShowValueBox, Editable
    Saved = True
    OnBeginEditText
End Sub
