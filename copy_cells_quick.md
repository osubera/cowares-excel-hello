...
# Downloads #
  ...
  * [downloads / ダウンロード](http://code.google.com/p/cowares-excel-hello/downloads/list?can=2&q=copy_cells_quick)
...
## 使い方 ##
  ...
  * 用途
    1. 指定セルから最後までのコピーを簡単に行う。
    1. 通常ブックとアドインの両形式が利用できる。アドイン名称は、`エクセルコピー セル範囲自動`。

  1. C16から最後までを、別のワークシートのB24以降にコピーする方法
    1. セルC16を選択し、ツールバーの `コピー元` ボタンを押す。
    1. コピー先シートのセルB24を選択する。
    1. ツールバーの `コピー` ボタンを押す。
  1. 上と同じ操作を行単位で行う
    1. ツールバーの `範囲の基準` リストで、 `最上行` を選ぶ。
    1. この後、上と同じ操作を行えば、コピー元シートのセルA16から最後までが、コピー先シートのA24以降にコピーされる。
  1. コピーの動作について
    1. 通常のコピー＆ペーストと同様に、数式や書式も含めたコピー動作となる。
    1. 同じ範囲を複数のシートに次々と貼り付ける場合、コピー元を毎回指定しなくてもよい。


# Snapshots #

**エクセル2000 ではツールバーを表示する。
> ![http://2.bp.blogspot.com/-Y1Puqzo-4HU/T_jq1704uLI/AAAAAAAAAKQ/dZs3KVy0oS0/s1600/shot2.png](http://2.bp.blogspot.com/-Y1Puqzo-4HU/T_jq1704uLI/AAAAAAAAAKQ/dZs3KVy0oS0/s1600/shot2.png)**

**エクセル2007 ではリボンのアドインタブにボタンを表示する。
> ![http://2.bp.blogspot.com/-afktJbuPXBE/T_jOF9qw9fI/AAAAAAAAAKE/2loPnhSI69A/s1600/shot1.png](http://2.bp.blogspot.com/-afktJbuPXBE/T_jOF9qw9fI/AAAAAAAAAKE/2loPnhSI69A/s1600/shot1.png)**
