...
# Downloads #
  ...
  * [downloads / ダウンロード](http://code.google.com/p/cowares-excel-hello/downloads/list?can=2&q=customer_support_mail)
...
## 使い方 ##
  ...
  * 用途
    1. 顧客からのメールを元に、注文情報をエクセルに転記する。
    1. 注文に応じた顧客宛てメールを、いくつかの雛形を元にして作成する。
  ...
  1. 注文メールの読み込み
    ...
    * ルール
      ...
      1. 【注文日時】：ハイフンをスペースに置き換え、日時をB列に入れる。
      1. 【 名　前 】：最後の「 様」を除外し、E列に入れる。
      1. 【 住　所 】：〒直後の最初の半角スペースの手前までをF列に、残りをG列に入れる。
      1. 【 ＴＥＬ 】： H列に入れる。
      1. 【 メールアドレス 】：I列に入れる。ハイパーリンクは生成しない。
      1. 【 お支払い方法 】：J列に入れる。
      1. 【 お客様ID‥半角数字４桁 】：K列に入れる。
      1. 【 ご希望の個数 】：最初の数字だけ（個の手前まで）をN列に入れる。残りをAA列に入れる。
      1. 【 備考 】：Z列に入れる。
    * 処理内容の確認ログ
    ...
    * 実行例
      1. メールソフトで、注文メールの対象箇所または本文全体を選択して、コピー。
      1. `クリップボードから読み込む` ボタンを押す。
        * 顧客シートの合計行の上にある空行（雛形行）をコピー挿入し、解釈した顧客情報等が書き込まれる。
        * シートのカーソルは、読み込んだ行に移動する。
        * メモ帳が起動され、解釈した情報のログを表示する。
      1. 解釈に問題があれば、メモ帳の情報を見ながら顧客シートを手修正し、問題がなければメモ帳を閉じる。
  1. 顧客宛てメールの作成
    * 動作
    ...
    * 実行例
      1. 顧客シートの、メールを出したい行にカーソルを置く。読み込み作業の直後なら、該当する行にカーソルがある。手入力した場合、自動で次の行にカーソルが進んでいる場合があるので、その場合、カーソル位置に注意し、正しい位置に戻しておく。
      1. 例文シートの、メール作成に使用する雛形列にカーソルを置く。この時点で、雛形に表示されている宛先等が古いまま（前回の送信先）になっているが、気にしなくてよい。
      1. `メールを作成`ボタンを押すと、メモ帳が起動し、作成したメールが表示される。
      1. メールソフトを手で起動し、メモ帳からコピーで転記してメールを作成し、送信する。
      1. メモ帳を閉じる。メモ帳はエクセルと連動していないので、不要になればいつ閉じてもよい。


# Snapshots #


**顧客シートにはクリップボードの情報を該当セルに格納する。
> ![http://2.bp.blogspot.com/-AGVkXxfJHlc/UHQap8ump7I/AAAAAAAAALQ/omy10bYqm0o/s1600/shot1.png](http://2.bp.blogspot.com/-AGVkXxfJHlc/UHQap8ump7I/AAAAAAAAALQ/omy10bYqm0o/s1600/shot1.png)**

**例文シートにメールの雛形を格納し、顧客シートの情報を差し込んでメール本文を作成する。
> ![http://3.bp.blogspot.com/-lNRbcYzROFQ/UHQatPhBYHI/AAAAAAAAALY/A8pc8dV5-QA/s1600/shot2.png](http://3.bp.blogspot.com/-lNRbcYzROFQ/UHQatPhBYHI/AAAAAAAAALY/A8pc8dV5-QA/s1600/shot2.png)**
...
【注文内容】受付番号：64
--------------------------------------------------------------
【注文日時】：2012/08/05-13:12
【 名　前 】：薔薇句　オバマ 様
【 住　所 】：〒012-3456 亜米利加国華盛頓州白家 1-1
【 ＴＥＬ 】： 00-1111-2222
【 メールアドレス 】：baraku@obama.x
【 お支払い方法 】：ホワイト銀行
【 お客様ID‥半角数字４桁 】：0001
【 ご希望の個数 】：6個(1セット‥10080円)→ 割引価格 8700円
【 備考 】：IDを記入しました。
...
【 お支払い方法 】：ホワイト銀行
【 お客様ID‥半角数字４桁 】：0001
【 ご希望の個数 】：x個(1セット‥10080円)→ 割引価格 8700円
【 備考 】：個数不適当。
