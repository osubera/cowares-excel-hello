...
# Details #
  ...
  * unfortunately, the Scripting.Dictionary object doesn't support sorting by keys.
    * referrence: [Sorting a Scripting Dictionary Populated with String Data](http://support.microsoft.com/kb/246067/en-us)
  * so we have to sort the key by ourselves, and the page offers a sample code of the `bubble sort`. let's test it.
  * we know the `bubble sort` is not practical on thousands of data for its speed, thus we provide the `merge sort` and the `quick sort` to compare performances.

## 説明 ##
  * 残念ながら、 Scripting.Dictionary オブジェクトでは、キーのソートができない。
    * 参考: [文字列データを設定するスクリプト辞書の並べ替え](http://support.microsoft.com/kb/246067/ja)
  * だから、キーを自分で並び替えないといけない。上の参考先では、 `バブルソート` のコードが提示されている。試してみよう。
  * `バブルソート` が千件単位のデータだと遅くて使えないのは知っているので、 `マージソート` と `クイックソート` も試して比べてみる。
...
Binary Sort    0.0078125     6.609375 
Merge Sort     0             0.5234375 
Quick Sort     0             0.1484375 

another result

Binary Sort    0             6.6875 
Merge Sort     0             0.5234375 
Quick Sort     0.0078125     0.140625 
